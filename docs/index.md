# DeepClaw UI — LLM-Optimized Reference
...
**Binary:** `deepclaw-ui.js` (Node.js server) + `index.html` (vanilla JS SPA)
**Port:** 1234 (default)
...
**Event flow:** Gateway → deepclaw-ui.js → browser (WebSocket) + REST polling fallback
...
| `DCPASS` | `deepclaw` | UI password (always enabled) |
...
HTTP Basic Auth **always required**. Default password: `deepclaw`.
...
| 401 Unauthorized | Use password `deepclaw` (or `DCPASS` env value) |
